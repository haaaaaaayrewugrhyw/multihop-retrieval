"""
ComplementGenerator (G+D) Full System Evaluation — retrieval_v3
===============================================================

Runs the same three-way comparison as retrieval_v2/run_eval.py
but uses the G+D ComplementGenerator instead of FakeEncoderModel.

Systems compared:
  1. MDR (dense, iterative)              — baseline, no model needed
  2. Graph + direct cosine (G)           — G passage embeddings, no complement
  3. FULL: G graph + complement + FAISS  — G+D complement edge vectors, full system

Interface (identical to FakeEncoderModel, so eval logic is unchanged):
  - passage encoding  : model.encode_query(text)         [mean-pool -> 128-dim]
  - edge complement   : model.extract_complement(A, B)   [G(A,B)    -> 128-dim]
  - checkpoint file   : generator_best.pt

All offline embeddings are cached to retrieval_v3/cache/ after first run.
...
# ── Shared utilities: sibling retrieval/ and retrieval_v2/ directories ────────
_RETRIEVAL_DIR = _HERE.parent / "retrieval"
_V2_DIR        = _HERE.parent / "retrieval_v2"
...
from generator_train import (                                                  # retrieval_v3/
    ComplementGenerator as FakeEncoderModel, MAX_A_LEN, MAX_B_LEN,
)
try:
    from model2_train import QueryEncoder                                       # retrieval_v3/
except ImportError:
    QueryEncoder = None
